...
```toml
[dependencies]
tallyowl-cluster-api = { path = "./tallyowl-cluster-api" } # TODO: point at the published/vendored crate
...
fn main() {
    let client = TallyOwlReplicationClient::new(HttpRpcCarrier::new("http://localhost:5080"));
    let resp = client.deliver_consensus(ConsensusMessage { group: GroupRef { kind: todo!(), name: None }, kind: todo!(), sender: "example".to_string(), generation: 0, payload: Vec::new() });
    match resp {
...
fn run_datagrams(carrier: &mut impl DatagramCarrier) -> Result<(), Box<dyn std::error::Error>> {
    let req = ConsensusMessage { group: GroupRef { kind: todo!(), name: None }, kind: todo!(), sender: "example".to_string(), generation: 0, payload: Vec::new() };
    // seq 0 marks an unsequenced datagram.
    carrier.send_datagram(&Datagram::new(OP_ORD, 0, encode_consensus_message(&req)).encode()?)?;

    if let Some(inbound) = carrier.recv_datagram()? {
        let dg = Datagram::decode(&inbound)?;
        let resp = decode_consensus_reply(&dg.payload)?;
