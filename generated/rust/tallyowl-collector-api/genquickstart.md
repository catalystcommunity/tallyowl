...
```toml
[dependencies]
tallyowl-collector-api = { path = "./tallyowl-collector-api" } # TODO: point at the published/vendored crate
...
fn main() {
    let client = TallyOwlCollectorClient::new(HttpRpcCarrier::new("http://localhost:5080"));
    let resp = client.submit_batch(SubmitBatchRequest { batch: Batch { batch_id: Vec::new(), items: Vec::new(), common_properties: None, sealed_at: 0, compression: None }, policy_version: None, protocol_version: None });
    match resp {
...
fn run_datagrams(carrier: &mut impl DatagramCarrier) -> Result<(), Box<dyn std::error::Error>> {
    let req = SubmitBatchRequest { batch: Batch { batch_id: Vec::new(), items: Vec::new(), common_properties: None, sealed_at: 0, compression: None }, policy_version: None, protocol_version: None };
    // seq 0 marks an unsequenced datagram.
    carrier.send_datagram(&Datagram::new(OP_ORD, 0, encode_submit_batch_request(&req)).encode()?)?;

    if let Some(inbound) = carrier.recv_datagram()? {
        let dg = Datagram::decode(&inbound)?;
        let resp = decode_submit_batch_response(&dg.payload)?;
