...
```toml
[dependencies]
tallyowl-control-api = { path = "./tallyowl-control-api" } # TODO: point at the published/vendored crate
...
fn main() {
    let client = TallyOwlControlClient::new(HttpRpcCarrier::new("http://localhost:5080"));
    let resp = client.run_query(QueryRequest { algebra_version: 0, consistency: todo!(), max_staleness_ms: None, budget: None, allow_partial: false, comparison_range: None, form: todo!(), node: None, funnel: None, retention: None, path: None, trace: None, timeline: None, attribution: None, campaign_summary: None });
    match resp {
...
fn run_datagrams(carrier: &mut impl DatagramCarrier) -> Result<(), Box<dyn std::error::Error>> {
    let req = QueryRequest { algebra_version: 0, consistency: todo!(), max_staleness_ms: None, budget: None, allow_partial: false, comparison_range: None, form: todo!(), node: None, funnel: None, retention: None, path: None, trace: None, timeline: None, attribution: None, campaign_summary: None };
    // seq 0 marks an unsequenced datagram.
    carrier.send_datagram(&Datagram::new(OP_ORD, 0, encode_query_request(&req)).encode()?)?;

    if let Some(inbound) = carrier.recv_datagram()? {
        let dg = Datagram::decode(&inbound)?;
        let resp = decode_query_response(&dg.payload)?;
