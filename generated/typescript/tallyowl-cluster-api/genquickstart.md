...
```sh
npm install tallyowl-cluster-api csilgen-transport
```
...
import { AsyncApiClient, type AsyncServiceTransport } from "tallyowl-cluster-api";
...
async function main() {
  ...
  const resp = await client.tallyOwlCluster.describeTopology({});
  console.log(resp);
}
...
import { toDescribeTopologyRequestCbor, fromTopologyResponseCbor } from "tallyowl-cluster-api";
import type { DescribeTopologyRequest, TopologyResponse } from "tallyowl-cluster-api";
...
async function main() {
  ...
  const req: DescribeTopologyRequest = {};
  await carrier.sendDatagram(new Datagram(OP_ORD, 0, toDescribeTopologyRequestCbor(req)).encode());
  ...
  if (inbound !== null) {
    ...
    const resp: TopologyResponse = fromTopologyResponseCbor(dg.payload);
