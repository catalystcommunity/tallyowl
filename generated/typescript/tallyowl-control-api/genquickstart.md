...
```sh
npm install tallyowl-control-api csilgen-transport
```
...
import { AsyncApiClient, type AsyncServiceTransport } from "tallyowl-control-api";
...
async function main() {
  ...
  const resp = await client.tallyOwlControl.runQuery({ algebraVersion: 0, consistency: undefined as any, allowPartial: false, form: undefined as any });
  console.log(resp);
}
...
import { toQueryRequestCbor, fromQueryResponseCbor } from "tallyowl-control-api";
import type { QueryRequest, QueryResponse } from "tallyowl-control-api";
...
async function main() {
  ...
  const req: QueryRequest = { algebraVersion: 0, consistency: undefined as any, allowPartial: false, form: undefined as any };
  await carrier.sendDatagram(new Datagram(OP_ORD, 0, toQueryRequestCbor(req)).encode());
  ...
  if (inbound !== null) {
    ...
    const resp: QueryResponse = fromQueryResponseCbor(dg.payload);
