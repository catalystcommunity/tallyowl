...
import { MAX_FRAME_DEFAULT, TransportError, validateMaxFrame } from "./conventions.js";
// Prefix `bytes` with a 4-byte big-endian length (CSIL stream framing), enforcing
// the max-frame guard before producing the frame.
export function frameLengthPrefixed(bytes, max = MAX_FRAME_DEFAULT) {
    validateMaxFrame(max);
    if (bytes.length > max)
        throw TransportError.frameTooLarge(bytes.length, max);
...
export class LengthPrefixedDeframer {
    ...
    max;
    constructor(max = MAX_FRAME_DEFAULT) {
    ...
    next() {
        ...
        if (len > this.max)
            throw TransportError.frameTooLarge(len, this.max);
...
export class LoopbackFrameCarrier {
    outbound = [];
    inbound = [];
    pushInbound(bytes) {
...
export class LoopbackDatagramCarrier {
    outbound = [];
    inbound = [];
    pushInbound(bytes) {
        this.inbound.push(bytes);
