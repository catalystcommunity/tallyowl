...
import {
  fromCaptureRequestCbor,
  toCaptureCriticalResponseCbor,
...
function seedstore() {
  ...
  const send = (async (url: unknown, init?: RequestInit) => {
    ...
    const frame = RpcRequest.decode(new Uint8Array(init!.body as ArrayBuffer));
    ops.push(frame.op);
    ...
    const body =
      frame.op === "capture-critical"
        ...
        : RpcResponse.ok(
            "CaptureResponse",
            toCaptureResponseCbor({ accepted: request.items.length }),
          ).withId(frame.id);
...
test("an unload flush reaches the host's own route and nothing else", async () => {
  ...
  const beaconed: { url: string; bytes: number }[] = [];
  const host = seedstore();
  const storefront = open(host.router);
  ...
  storefront.client.capture(
    (await import("../../../packages/browser/src/index.ts")).Capture.event("tab-closing"),
  );
  flushNow();
